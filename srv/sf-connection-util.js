...
async function getEmployeeTime(
    ...
    const destination_sf = await cds.connect.to('destination_sf')
    let result = await destination_sf.send({ 
        query: `GET /odata/v2/EmployeeTime?&$format=json&$filter=userId eq '${userId}' and approvalStatus ${approvalStatusOperator} '${approvalStatus}' and startDate gt datetime'${startDate}' and endDate lt datetime'${endDate}' and ( timeType eq '${timeType}' or timeType eq 'AnnualLeaveD' or timeType eq 'LOATT' )`, 
        headers: { Authorization: AUTHORIZATION_HEADER } 
    })
    if(result?.d?.results){
